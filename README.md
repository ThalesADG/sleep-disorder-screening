# Triagem de Distúrbios do Sono (MVP) 😴

> 🟢 **Acesse o Sistema Online:** [Clique aqui para testar o App](https://sleep-disorder-screening-deploy-vuqhvnqp9e2jjvl5tghs7u.streamlit.app/)

Projeto de triagem automática desenvolvido para a Seleção da Ligia 2026.
O sistema recebe dados clínicos e prevê riscos de Insônia ou Apneia.

## 👥 Equipe
- Ivison Rafael
- Mariana Beatriz
- Mirella Fontinelle
- Paulo Teodoro
- Pedro Bezerra
- Thales Afonso

## 📑 Atribuições
- 🔎 **Análise de Dados:** Ivison Rafael, Mirella Fontinelle, Mariana Beatriz
- 🧠 **Modelagem:** Paulo Teodoro, Mirella Fontinelle, Ivison Rafael
- 🖥️ **Front-end (PoC):** Thales Afonso, Pedro Bezerra
- 🎥 **Vídeo:** Paulo Teodoro, Mirella Fontinelle, Ivison Rafael
- 📄 **Relatório:** Thales Afonso, Mirella Fontinelle, Mariana Beatriz

## ▶️ Execução do Projeto
1. 📥 Clone o repositório.
2. 📦 Instale as dependências: `python -m pip install -r requirements.txt`
3. ▶️ Execute o aplicativo: `python -m streamlit run app.py`

# ℹ️ Informações Complementares

## 📚 Notebooks Colab
- 🔍 **EDA:** [Colab](https://drive.google.com/file/d/1IBf0hNjq00lFcrAqjPxLdrtvyr-vOaXa/view?usp=sharing)
- 🧪 **Desenvolvimento:** [Colab](https://colab.research.google.com/drive/1ZvHcHiQM4VPUL8MGKM7PfqMJltERJfMr?usp=drive_link)
- ⚙️ **Inferência:** [Colab](https://colab.research.google.com/drive/1eSu_BYy89-GnoMUBzBcXp51ZIUPj8c8n?usp=drive_link)

*Obs.: Os notebooks também estão disponíveis no repositório na pasta `notebooks`.*

## 🔗 Referências e Entregáveis
- 🎥 **Vídeo:** [Link](https://drive.google.com/file/d/17_8ysheBKVJgaZYGyydcFlAkiZYbl3Zk/view?usp=drive_link) 
- 📄 **Relatório:** [Link](https://drive.google.com/file/d/1ECFPDvJJ4MTABwMi-tt6hdX1b_m56ijL/view?usp=drive_link) 
